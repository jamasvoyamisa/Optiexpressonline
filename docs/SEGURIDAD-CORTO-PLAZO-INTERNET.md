...
| No indexar en buscadores (SPA React) | Pendiente (intranet estática ya tiene `noindex`) |
| Límite por usuario / bloqueo de cuenta | Pendiente |
| 2FA | Pendiente |
| Alertas de fuerza bruta | Pendiente |
...
### A1. Rate limit por IP **y** por usuario
...
### A2. Bloqueo temporal de cuenta tras N fallos
...
### A3. Alertas de fuerza bruta a TI
...
### A4. CAPTCHA solo tras varios fallos (opcional pero recomendado en internet)
...
### B1. 2FA para Administrador / Superuser / RH
...
### B2. Revisar duración del access token

**Estado actual:** access token ~ **8 horas** (`ACCESS_TOKEN_EXPIRE_MINUTES = 480`).

**Propuesta de corto plazo:**
- Mantener 8 h para jornada laboral **o** bajar a 2–4 h + refresh silencioso (ya existe refresh token).
- Confirmar que el refresh también valida `session_id` (un solo dispositivo activo por usuario).

**Impacto usuario:** Bajo si el refresh funciona bien.  
**Impacto seguridad:** Medio (ventana de robo de token más corta).

### B3. Política mínima de contraseña al cambiar
...
### C1. Pasar CSP de Report-Only a enforce (cuando no haya violaciones)

**Estado:** `Content-Security-Policy-Report-Only` activo.

**Propuesta:**
- Revisar 3–7 días si hay violaciones legítimas.
- Si limpio: cambiar a `Content-Security-Policy` (modo forzado).

### C2. Rate limit / fail2ban en Nginx
...
### C3. Headers de seguridad adicionales

Verificar / añadir si faltan:
- `X-Frame-Options: SAMEORIGIN` (o `frame-ancestors` vía CSP)
- `X-Content-Type-Options: nosniff`
- `Referrer-Policy: strict-origin-when-cross-origin`
- `Permissions-Policy` básico

### C4. WAF ligero (Cloudflare u otro) — si el dominio lo permite
...
### C5. No indexar la aplicación en buscadores

**Problema:** Al vivir en internet, Google/Bing pueden listar la URL del login o de la gestión interna. Eso no abre el sistema por sí solo, pero aumenta la visibilidad y los “descubrimientos” casuales.

**Qué aporta:** Privacidad / discreción (menos aparición en resultados de búsqueda).  
**Qué no aporta:** No sustituye rate limit, bloqueo de cuenta ni 2FA. Quien ya tenga la URL (o la descubra por otros medios) sigue pudiendo intentar acceso.

**Estado actual:**
- Intranet estática (`intranet optiexpress/*.html`): ya tiene `<meta name="robots" content="noindex, nofollow">`.
- App React (`frontend/index.html`): **sin** meta robots ni header `X-Robots-Tag`.
...
| Orden | Ítem | Esfuerzo | Riesgo deploy | Valor |
|------:|------|----------|---------------|-------|
...
**Primera entrega sugerida (esta semana):** A1 + A2 + A3 + C3 + C5.

---

## Criterios de aceptación (primera entrega)

1. Más de 5 fallos seguidos en la misma cuenta → no permite login durante 15 min.
2. Más de 10 intentos/min desde la misma IP en login → HTTP 429.
3. Cada bloqueo o ráfaga queda en `actividad_log`.
4. Login correcto sigue mostrando el mismo flujo (incluido `must_change_password`).
5. Portal remoto (`/portal/checadas`, `/portal/estado-hoy`) aplica las mismas reglas de abuso.
...
8. La SPA responde con `noindex` (meta y/o `X-Robots-Tag`) y `robots.txt` no permite rastreo.
...
- Auditoría completa de módulos de negocio no relacionados con auth.
