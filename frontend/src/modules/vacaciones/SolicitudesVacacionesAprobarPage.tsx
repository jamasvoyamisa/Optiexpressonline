...
export const SolicitudesVacacionesAprobarPage = () => {
  ...
  const cargarDatos = useCallback(async () => {
    ...
    setError(null);
    try {
      const [vacRes, presRes] = await Promise.all([
        (async () => {
        ...
        api.get<SolicitudPrestamo[]>('prestamos/solicitudes-pendientes', { params: { limit: 500 } }).then(r => Array.isArray(r.data) ? r.data : []).catch(() => []),
      ]);
      setSolicitudes(vacRes);
      setSolicitudesPrestamos(presRes);

      const idsVac = [...new Set(vacRes.map(s => s.empleado_id))];
      const idsPres = [...new Set(presRes.map(s => s.empleado_id))];
  ...
  const handleAprobarRechazarPrestamo = (aprobar: boolean) => {
    ...
    setAprobando(true);
    api.post(`prestamos/${modalAprobarPrestamo.id}/aprobar`, {
      aprobado: aprobar,
      comentarios: aprobacionComentarios.trim() || null,
    })
      .then(() => {
  ...
  const totalPendientes = activeTab === 'vacaciones' ? solicitudes.length : solicitudesPrestamos.length;
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      ) : activeTab === 'prestamos' && solicitudesPrestamos.length === 0 ? (
        <div style={{
          padding: 48,
          textAlign: 'center',
          backgroundColor: '#f9fafb',
              ...
              {solicitudesPrestamos.map(s => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ ...td, textAlign: 'center' }}>{s.plazo_meses} meses</td>
                    <td style={{ ...td, maxWidth: 180 }}>{s.motivo || '—'}</td>
                    <td style={{ ...td, textAlign: 'center' }}>
                      <button
      ...
      {modalAprobarPrestamo && (
        ...
        >
          <div
            ...
            role="dialog"
          >
            <h2 style={{ marginTop: 0, marginBottom: 12 }}>Aprobar o rechazar préstamo</h2>
            <p style={{ color: '#555', marginBottom: 6, fontWeight: 600 }}>
              {nombreEmp(empleadosMap.get(modalAprobarPrestamo.empleado_id))}
            </p>
            <p style={{ color: '#555', marginBottom: 14, fontSize: '0.9rem' }}>
              {formatMonto(modalAprobarPrestamo.monto)} a {modalAprobarPrestamo.plazo_meses} meses · Pendiente confirmación RH
            </p>
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: 'block', fontWeight: 600, marginBottom: 6, fontSize: '0.9rem' }}>Comentarios (opcional)</label>
              <textarea
