...
def upgrade():
    conn = op.get_bind()
    cols = [row[0] for row in conn.execute(sa.text("SHOW COLUMNS FROM empleado_horario")).fetchall()]
    if 'hora_salida_sabado' not in cols:
        op.add_column('empleado_horario', sa.Column('hora_salida_sabado', sa.String(10), nullable=True))
